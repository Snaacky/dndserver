...
def is_banned_user(username: str, session: Session) -> bool:
    banned_user = session.query(Account).filter(Account.username == username, Account.ban_type != None).first()
...
def process_login(ctx, msg):
    ...
    # Check if the HWID is banned or the user is banned
    if is_banned_hwid(req.hwIds[0], db) or is_banned_user(req.loginId, db):
        if is_banned_hwid(req.hwIds[0], db):
            #should be res.FAIL_LOGIN_BAN_HARDWARE but Not all SS2C_ACCOUNT_LOGIN_RES fields are implemented
            res.Result = res.FAIL_PASSWORD
        else:
            #should be res.FAIL_LOGIN_BAN_USER but Not all SS2C_ACCOUNT_LOGIN_RES fields are implemented.
            res.Result = res.FAIL_PASSWORD
        return res
